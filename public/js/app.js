(function(){
	"use strict";
	var app = {
		menuJson : $.ajax("http://192.168.1.113:1337/menu.json"),
		
		init:function(){
			app.listener();
		},
		markdone: function(){
			var markdown = $.ajax("http://192.168.1.113:1337/example.md")
			markdown.done(function(){
				$.get("http://192.168.1.113:1337/example.md",function(data){
		...
		menuDone : function(){
			app.menuJson.done(function(menuJ){
				for (var i = 0; i < menuJ.menu.length; i++) {
					...
					var pathJson = "http://192.168.1.113:1337"+menuJ.menu[i].path;
